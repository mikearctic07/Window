...
void PORTC_IRQHandler(void)
{
	...
	else if(*vectorInterruption==UP_INTERRUPTION)  /*Checks if the interrupt is generated by SW2 (PTC12)*/
	{
		...
		while((PTC->PDIR & (1<<PTC12)) && (*vectorInterruption ==0 )&&(currentLedState<FIRST_10_BITS)) /*Checks if the SW2 is being pressed and the ISFR register is 0 (empty)*/
		{
			movingUpFlag=1;
		...
		if(*vectorInterruption == ANTIPINCH_INTERRUPTION)
		{
			movingUpFlag=1;
		}
		else
	...
	else								       /*Checks for pin 13 on Port C has generated the interrupt*/
	{
		...
		while((PTC->PDIR & (1<<PTC13)) && (currentLedState>0))     /*checks if pin13 Port C (SW3) is being pressed*/
		{
